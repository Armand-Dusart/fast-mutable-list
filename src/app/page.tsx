...
import { FastList } from "./ui/components/FastList";
import { SlowList } from "./ui/components/SlowList";

export default function Home() {
  const initList = new Array(10000).fill(0).map((_, i) => ({
    label: "",
    id: i,
    ...
    <div className={"p-4 w-full h-full"}>
      <div className={"max-w-5xl mx-auto relative"}>
        <div className="text-base-content text-3xl z-10">
          <span>
            The first render will be painful. To overcome this problem
            virtualization is necessary. When you change the input value, you
            will see the difference between the two lists.
          </span>
        </div>
        <div className="absolute top-24 -right-24 pointer-events-none drop-shadow-lg">
          ...
          >
            ...
            <defs>
              ...
              >
                <stop stopColor="#4c35fd" />
                <stop offset="1" stopColor="#808ff6" />
              </linearGradient>
        ...
        <div className="relative z-10 flex mt-12 justify-between">
          <div>
            <SlowList initList={initList} />
          </div>
          <div>
            <FastList initList={initList} />
          </div>
        </div>
      </div>
    </div>
